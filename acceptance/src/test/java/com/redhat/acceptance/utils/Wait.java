...
public class Wait{
	  public static boolean For(String errorMessage, int timeoutInSeconds, int intervalInSeconds, ToHappen toHappen) {
...
//	      System.out.println("[Wait] - waiting... ["+((end-System.currentTimeMillis())/1000)+"s]");
	  ...
	  public static boolean For(String errorMessage, int timeoutInSeconds, ToHappen toHappen) {
	    return For(errorMessage, timeoutInSeconds, 1, toHappen);
	  }

    public static boolean For(int timeoutInSeconds, ToHappen toHappen) {
      return For("unspecified wait", timeoutInSeconds, 1, toHappen);
    }
}