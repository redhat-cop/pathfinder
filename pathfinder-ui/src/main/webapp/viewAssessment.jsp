	...
	<section class="wrapper">
		<div class="inner">
								...
								function() {
									httpGetObject(
											Utils.SERVER
													...
													+ "/applications/"
													+ appId
													+ "?custom=assessment.NOTES,customer.name",
											function(application) {
												document
														.getElementById("applicationName").innerHTML = application.Name;
												document
														.getElementById("applicationDescription").innerHTML = application.Description != "" ? application.Description
														: "No description provided";
												document
														.getElementById("assessmentNotes").innerHTML = application.CustomFields['assessment.NOTES'] != undefined ? application.CustomFields['assessment.NOTES']
														: "None";

												console
														.log("application.CustomFields = "
																+ JSON
																		.stringify(application.CustomFields));

												if (undefined != initTabs) {
													initTabs(
															"assessments",
															customerId,
															application.CustomFields['customer.name']);
												}

												if (null != application['Review']
														&& Utils
																.getParameterByName("review") != null) {
													httpGetObject(
															Utils.SERVER
																	+ '/api/pathfinder/customers/'
																	+ customerId
									...
									function(event) {
										...
										if (activePoints.length > 0) {
											...
											if (currentSelectedPieceLabel
												...
												if (clickedLabel.toUpperCase() == currentSelectedPieceLabel
														.toUpperCase()) {
			...
			<div class="row title-row">
				...
				<div class="col-sm-8">
					<h2>
						<span id="applicationName">Loading...</span>
					</h2>
				</div>
			</div>
											...
											httpGetObject(
													...
													function(
														...
														for (i = 0; i < data.length; i++) {
															if (newdata[data[i]['rating']] == undefined)
																newdata[data[i]['rating']] = 0;
																...
																.DataTable(
																		{
																			...
																			"columnDefs" : [ {
																				...
																				"render" : function(
																						...
																						type,
																						row) {
																					return "<span style='color:"+Utils.chartColors[row["rating"]]+"'>"
																							+ row['rating']
																							+ "</span>";
																				}
																			} ]
						...
						<div class="col-sm-12">
							...
							<div class="row">
								...
								<div class="col-sm-10">
									...
									<p>Please use this section to provide your assessment of
										the possible migration/modernisation plan and an effort
										estimation.</p>

									<form
										action="/api/pathfinder/customers/<%=request.getParameter("customer")%>/applications/<%=request.getParameter("app")%>/"
										id="form" method="post">
										...
										<div class="row">
											...
											<div class="col-sm-2">
												<select type="text" id="BusinessPriority"
										...
										<script>
											function postReview(formId) {
												...
												var callback = new function(
													...
													console
															.log("Callback::after post: response= "
																	+ response);
												...
												xhr.onloadend = function() {
													...
													if (xhr.status == 200) {
														console
																.log("PostWait::http 200 returned ok, redirecting...");
														window.location.href = "assessments.jsp?customerId="
																+ customerId;
													}
												};
											}
